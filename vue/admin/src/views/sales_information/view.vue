		...
		<el-row class="row_ce"> 
					...
					<div v-else-if="$check_field('get','sales_order_number')">{{form['sales_order_number']}}</div>
							...
							<el-select v-if="(form['sales_information_id'] && $check_field('set','user_id')) || (!form['sales_information_id'] && $check_field('add','user_id'))" id="user_id" v-model="form['user_id']" :disabled="disabledObj['user_id_isDisabled']">
								<el-option v-for="o in list_user_user_id" :key="o['user_name']" :label="o['nick_name'] + '-' + o['user_name']"
										   :value="o['user_id']">
								</el-option>
							</el-select>
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_btn_warp">
				<el-form-item v-if="$check_action('/sales_information/view','set') || $check_action('/sales_information/view','add') || $check_action('/sales_information/table','set') || $check_action('/sales_information/table','add')">
