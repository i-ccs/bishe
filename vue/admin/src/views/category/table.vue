			...
			<el-row class="rows row2">	
				<el-col :xs="24" :sm="24" :lg="24" class="search_btn_wrap">
					<el-col :xs="24" :sm="12" :lg="12" class="search_btn_1 btns">
						...
						<el-button v-if="$check_action('/category/table','del')" class="float-right search_btn_del" type="danger" @click="delInfo()">删除</el-button>
					
						<el-button  v-if="$check_action('/category/view')" class="float-right  search_btn_add" @click="$router.push('./view?')">添加</el-button>
					</el-col>
...
<script>
	...
	export default {
		...
		data() {
			return {
				// 获取数据地址
				url_get_list: "~/api/category/get_list?like=0",
				url_del: "~/api/category/del?",
