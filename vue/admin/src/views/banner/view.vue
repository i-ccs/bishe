<template>
	<el-main class="bg edit_wrap banner">
		<el-card class="premium-view-card" shadow="never">
			<div slot="header" class="premium-header">
				<span class="premium-title">{{ form.banner_id ? '编辑轮播图' : '添加轮播图' }}</span>
			...
			<el-form ref="form" :model="form" status-icon label-width="80px" label-position="top">
				...
				<div class="premium-actions">
					<template v-if="user_group == '管理员' || (form['banner_id'] && $check_action('/banner/view','set')) || (!form['banner_id'] && $check_action('/banner/view','add'))">
	...
	export default {
		...
		data() {
			return {
				table: "banner",
				field: "banner_id",
				url_add: "~/api/banner/add?",
				url_set: "~/api/banner/set?",
				url_get_obj: "~/api/banner/get_obj?",
				url_upload: "~/api/banner/upload?",
				query: { banner_id: 0 },
				form: {
					banner_id: 0,
					title: "",
