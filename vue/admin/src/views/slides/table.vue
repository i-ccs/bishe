	...
	<el-main class="bg table_wrap premium-table-wrap">
		...
		<el-card class="premium-table-card" shadow="never">
			<div class="premium-table-btns">
				<el-button v-if="user_group == '管理员' || $check_action('/slides/view')" type="primary" icon="el-icon-plus" @click="$router.push('./view?')">新增轮播图</el-button>
				<el-button v-if="user_group == '管理员' || $check_action('/slides/table','del')" type="danger" icon="el-icon-delete" @click="delInfo()" :disabled="selection.length === 0">批量删除</el-button>
			</div>
	...
	export default {
		...
		data() {
			return {
				...
				query: { size: 10, page: 1, title: "", orderby: `slides_id desc` },
				list: [],
			}
		},
		methods: {
			reset() {
				this.query = { size: 10, page: 1, title: "", orderby: `slides_id desc` };
				this.get_list();
			}
		}
	}
