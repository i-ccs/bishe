<template>
	...
	<el-row class="rows row2" type="flex" justify="center">
			...
			<el-table-column fixed="right" label="操作" v-if="$check_action('/purchasing_information/table','set') || $check_action('/purchasing_information/view','set') || $check_action('/purchasing_information/view','get') 
				...
				<template slot-scope="scope">
					<div class="view_a">
					<router-link class="e-button el-button--small is-plain el-button--success" style="margin: 5px !important;" v-if="$check_action('/purchasing_information/table','set') || $check_action('/purchasing_information/view','set') || $check_action('/purchasing_information/view','get')" :to="'./view?' + field + '=' + scope.row[field]" size="small">
						<span>详情</span>
					</router-link>
					<el-button v-if="user_group == '管理员' && scope.row.procurement_status !== '已审批'" class="e-button el-button--small is-plain el-button--warning" style="margin: 5px !important;" @click="approveInfo(scope.row)">
						<span>审批</span>
					</el-button>
				</div>
				</template>
