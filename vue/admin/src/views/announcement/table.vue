	...
	<el-main class="bg table_wrap premium-table-wrap">
		...
		<el-card class="premium-table-card" shadow="never">
			<div class="premium-table-btns">
				<el-button v-if="$check_action('/announcement/view')" type="primary" icon="el-icon-plus" @click="$router.push('./view?')">发布公告</el-button>
				<el-button v-if="$check_action('/announcement/table','del')" type="danger" icon="el-icon-delete" @click="delInfo()" :disabled="selection.length === 0">批量删除</el-button>
			</div>
...
<script>
	...
	export default {
		...
		data() {
			return {
				url_get_list: "~/api/announcement/get_list?like=0",
				url_del: "~/api/announcement/del?",
