	...
	<el-main class="bg edit_wrap comtable_e">
		...
		<el-row class="row_ce"> 
							...
							<el-form-item label="商品编码" prop="product_code">
							  ...
							  v-if="(form['merchandise_information_id'] && $check_field('set','product_code')) || (!form['merchandise_information_id'] && $check_field('add','product_code'))" :disabled="true"></el-input>
					<div v-else-if="$check_field('get','product_code')">{{form['product_code']}}</div>
							...
							<el-form-item label="商品名称" prop="product_name">
							  ...
							  v-if="(form['merchandise_information_id'] && $check_field('set','product_name')) || (!form['merchandise_information_id'] && $check_field('add','product_name'))" :disabled="disabledObj['product_name_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','product_name')">{{form['product_name']}}</div>
							...
							<el-form-item label="商品类别" prop="product_category">
							  ...
							  v-if="(form['merchandise_information_id'] && $check_field('set','product_category')) || (!form['merchandise_information_id'] && $check_field('add','product_category'))" :disabled="disabledObj['product_category_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','product_category')">{{form['product_category']}}</div>
							...
							<el-form-item label="商品品牌" prop="product_brand">
							  ...
							  v-if="(form['merchandise_information_id'] && $check_field('set','product_brand')) || (!form['merchandise_information_id'] && $check_field('add','product_brand'))" :disabled="disabledObj['product_brand_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','product_brand')">{{form['product_brand']}}</div>
							...
							<el-form-item label="商品规格" prop="merchandise_specifications">
							  ...
							  v-if="(form['merchandise_information_id'] && $check_field('set','merchandise_specifications')) || (!form['merchandise_information_id'] && $check_field('add','merchandise_specifications'))" :disabled="disabledObj['merchandise_specifications_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','merchandise_specifications')">{{form['merchandise_specifications']}}</div>
						...
						v-if="(form['merchandise_information_id'] && $check_field('set','product_inventory')) || (!form['merchandise_information_id'] && $check_field('add','product_inventory'))" :disabled="disabledObj['product_inventory_isDisabled']"></el-input-number>
					<div v-else-if="$check_field('get','product_inventory')">{{form['product_inventory']}}</div>
						...
						:show-file-list="false" v-if="(form['merchandise_information_id'] && $check_field('set','product_images')) || (!form['merchandise_information_id'] && $check_field('add','product_images'))">
					...
					<el-image v-else-if="$check_field('get','product_images')" style="width: 100px; height: 100px"
						...
						v-if="(form['merchandise_information_id'] && $check_field('set','merchandise_price')) || (!form['merchandise_information_id'] && $check_field('add','merchandise_price'))" :disabled="disabledObj['merchandise_price_isDisabled']"></el-input-number>
					<div v-else-if="$check_field('get','merchandise_price')">{{form['merchandise_price']}}</div>
						...
						v-if="(form['merchandise_information_id'] && $check_field('set','product_introduction')) || (!form['merchandise_information_id'] && $check_field('add','product_introduction'))" :disabled="disabledObj['product_introduction_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','product_introduction')">{{form['product_introduction']}}</div>
						...
						v-if="(form['merchandise_information_id'] && $check_field('set','product_details')) || (!form['merchandise_information_id'] && $check_field('add','product_details')) ">
					</quill-editor>
			  ...
			  <el-form-item label="销售限次">
				<el-input id="limit_times" v-model="form['sales_information_limit_times']" placeholder="销售限制次数，0为不限"
						  v-if="$check_option('/merchandise_information/view','can_limits')"
				></el-input>
				<div v-else-if="$check_action('/merchandise_information/view','get')" v-html="form['sales_information_limit_times']"></div>
			  </el-form-item>
			</el-col>
					<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp" v-if="!form['source_user_id'] && 0 > 0">
			  <el-form-item label="采购限次">
				<el-input id="limit_times" v-model="form['purchasing_information_limit_times']" placeholder="采购限制次数，0为不限"
						  v-if="$check_option('/merchandise_information/view','can_limits')"
				></el-input>
				<div v-else-if="$check_action('/merchandise_information/view','get')" v-html="form['purchasing_information_limit_times']"></div>
			  </el-form-item>
			</el-col>
					<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp" v-if="!form['source_user_id'] && 0 > 0">
			  <el-form-item label="库存限次">
				<el-input id="limit_times" v-model="form['inventory_information_limit_times']" placeholder="库存限制次数，0为不限"
						  v-if="$check_option('/merchandise_information/view','can_limits')"
				></el-input>
				<div v-else-if="$check_action('/merchandise_information/view','get')" v-html="form['inventory_information_limit_times']"></div>
			  </el-form-item>
			</el-col>
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_btn_warp">
				<el-form-item v-if="$check_action('/merchandise_information/view','set') || $check_action('/merchandise_information/view','add')">
	...
	export default {
		...
		data() {
			return {
				...
				url_add: "~/api/merchandise_information/add?",
				url_set: "~/api/merchandise_information/set?",
				url_get_obj: "~/api/merchandise_information/get_obj?",
				url_upload: "~/api/merchandise_information/upload?",
				...
				form: {
								"product_code": this.$get_stamp(), // 商品编码
										...
										"product_details":  '', // 商品详情
											"merchandise_information_id": 0, // ID
		...
		methods: {
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/merchandise_information/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/merchandise_information/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/merchandise_information/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/merchandise_information/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/merchandise_information/view','get');
